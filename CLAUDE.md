...
**部署與版本紀錄的順序（使用者明確要求的工作流程）：**
1. 本機驗證通過後直接 `git push` 上線，不用每次都先問過使用者
2. **不要**在 push 的同時更新 `CHANGELOG.md`——版本紀錄要等使用者自己上線確認沒問題、明確說「更新版本紀錄」才寫
3. `CHANGELOG.md` 格式參考同一使用者的 daily_planner_2026 專案：`## vX.Y — 標題（日期）` + 白話條列說明，大版號＝重要新功能／介面改版，小版號＝改善或修正
